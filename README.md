...
2. 逐片:连接该分片房间 → 写入 64KB base64 块 → **等待发送缓冲清空 + 留时间给服务端持久化** → 断开 → 下一片;
3. 所有分片落盘后,**最后**才往主房间写 `files` 元数据。

> 元数据最后写是关键:别人一旦在列表看到文件,其所有分片必然已持久化完成。

**下载 / 预览**(`store.readFile`,异步):读元数据拿到 `shards` 数 → **并发下载分片房间(工作池,默认 4 路并行;`DL_CONCURRENCY` 可调):各自连接 → 同步 → 取块 → 断开** → 结果按序号归位、解码 base64 拼回完整 `Uint8Array` → 包成 `Blob` 触发下载或喂给预览组件,带进度回调。
